...
export default function AdminTranslationsPage() {
  ...
  const [editSectionsError, setEditSectionsError] = useState("");
  ...
  function startEdit(t: any) {
    ...
    // Load section overrides from content.sections
    const sections = t.content?.sections;
    setEditSections(sections ? JSON.stringify(sections, null, 2) : "{}");
    setEditSectionsError("");
  }

  async function saveEdit(id: string) {
    if (!editName || !editSlug || !supabase) return;
    // Parse section overrides JSON
    let sectionsData: any = {};
    try { sectionsData = JSON.parse(editSections); setEditSectionsError(""); }
    catch (e: any) { setEditSectionsError("Invalid JSON: " + e.message); return; }
    // Read current content, merge sections
    const { data: cur } = await supabase.from("product_translations").select("content").eq("id", id).single();
    const mergedContent = { ...(cur?.content || {}), sections: Object.keys(sectionsData).length > 0 ? sectionsData : undefined };
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
            {[
              ...
              ["de","🇩🇪","Deutsch"],["no","🇳🇴","Norsk"],["fi","🇫🇮","Suomi"],["sv","🇸🇪","Svenska"]
            ].map(([code,flag,label]) => (
              <button key={code} onClick={() => { setLocale(code); setEditing(null); }}
                className={`px-4 py-2 text-sm tracking-wider rounded-lg border transition-all ${
                  locale === code ? "bg-forest text-deep-dark border-forest" : "bg-transparent text-silver border-silver/30 hover:border-silver hover:text-white"
                }`}>{flag} {label}</button>
            ))}
          </div>
        </div>
          ...
          <div className="mt-6 bg-deep-blue/30 border border-silver/10 rounded-xl p-6">
            ...
            <div className="space-y-3 max-w-2xl">
              ...
              <div className="pt-4 border-t border-silver/10 mt-4">
                <p className="text-[10px] text-silver/40 uppercase tracking-wider mb-2">📋 Product Sections (JSON)</p>
                <p className="text-[10px] text-silver/40 mb-2">Translate highlights, param counters, spec tabs and eco features. Keys: "highlights", "paramCounters", "specTabs", "ecoFeatures".</p>
                {editSectionsError && <p className="text-red-400 text-xs mb-2">{editSectionsError}</p>}
                <textarea value={editSections} onChange={(e) => setEditSections(e.target.value)}
                  className="w-full bg-deep-dark border border-silver/10 rounded px-3 py-2 text-xs text-white font-mono"
                  style={{minHeight:"200px"}} placeholder='{"highlights":["Feature 1","Feature 2"]}' />
              </div>
