...
const COLORS = [
  ...
  { label: "Forest Green", value: "#009f4b" },
    ...
    <div className="flex">
      ...
      <main className="ml-64 flex-1 p-8">
        ...
        <div className="flex gap-2 mb-6 flex-wrap items-center">
          ...
          {[
            ["en","🇬🇧","EN"],["zh","🇨🇳","ZH"],["fr","🇫🇷","FR"],
            ["de","🇩🇪","DE"],["no","🇳🇴","NO"],["fi","🇫🇮","FI"],["sv","🇸🇪","SV"]
          ].map(([code,flag,label]) => (
            <button key={code} onClick={() => setLocale(code)}
              className={`px-3 py-1.5 text-xs rounded border ${locale === code ? "bg-ice/20 text-ice border-ice/30" : "text-silver/50 border-silver/20"}`}>{flag} {label}</button>
          ))}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button type="button" onClick={() => addBlock("raw-html")} className="px-3 py-1.5 text-xs bg-amber-400/20 text-amber-400 border border-amber-400/30 rounded hover:bg-amber-400/30 transition-colors">📝 Raw HTML</button>
            ...
            <div className="space-y-3">
              {blocks.map((block, i) => (
                <div key={i} className="bg-deep-blue/30 border border-silver/10 rounded-xl p-4">
                  ...
                  {block.type === "raw-html" && (
                    <div className="space-y-2">
                      <p className="text-[10px] text-silver/40">Paste Figma/ChatGPT/any HTML here. Rendered as-is with no style sanitization.</p>
...
function BlockPreview({ block }: { block: Block }) {
  ...
  switch (block.type) {
    ...
    case "raw-html":
      return <div className="text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: block.data?.html || "" }} />;
    case "rich-text": {
      const segs = block.data?.segments || [];
      return (
