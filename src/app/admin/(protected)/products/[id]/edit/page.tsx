      ...
      <main className="ml-64 flex-1 p-8">
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
          ...
          <div className="bg-deep-blue/30 border border-silver/10 rounded-xl p-6">
            ...
            <ImageUploader existing={images.map((i: any) => i.url || i)} onUploaded={(urls: string[]) => setImages(urls.map(u => {
              const existing = images.find((img: any) => (img.url || img) === u);
              return { url: u, alt: existing?.alt || "" };
            }))}  />
            {images.length > 0 && (
