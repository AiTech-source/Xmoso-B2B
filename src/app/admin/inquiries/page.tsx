      ...
      <main className="ml-64 flex-1 p-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-light tracking-wider text-white">
            📨 Inquiries
            <span className="text-sm text-silver/40 ml-3 font-normal">{inquiries.length} total</span>
          </h1>
            ...
            <tbody>
              {inquiries.map((inq) => (
                <tr key={inq.id}
                  ...
                  className={`border-b border-silver/5 hover:bg-white/5 cursor-pointer transition-colors ${inq.is_read ? "" : "bg-forest/[0.03]"}`}>
              ...
              {inquiries.length === 0 && (
                <tr><td colSpan={7} className="p-8 text-center text-xs text-silver/40">No inquiries yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
