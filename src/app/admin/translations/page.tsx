...
export default function AdminTranslationsPage() {
  ...
  // Load all products for create-translation dropdown
  useEffect(() => {
  ...
  async function deleteTranslation(id: string) {
    if (!confirm("Delete this translation? This cannot be undone.") || !supabase) return;
  ...
  async function createTranslation() {
    ...
    const { data: existing } = await supabase
      ...
      .eq("slug", createSlug);
    if (existing?.length) {
      alert(`Slug "${createSlug}" already exists for locale "${locale}".`);
      return;
    }
    ...
    <div className="flex">
      ...
      <main className="ml-64 flex-1 p-8">
          ...
          <div className="flex gap-2">
            {(["en", "zh"] as const).map((l) => (
              <button key={l} type="button" onClick={() => { setLocale(l); setEditing(null); }}
                className={`px-4 py-2 text-sm tracking-wider rounded-lg border transition-all duration-300 ${
                  locale === l
        ...
        <p className="text-xs text-silver/40 mb-6">
          Edit product names, descriptions, and SEO metadata per language.
          {locale === "zh" && (
            <span className="text-amber-400/70 ml-2">
              💡 If a product has no CN translation yet, the EN version will be shown automatically.
            </span>
          )}
        </p>
        ...
        {creating && (
          <div className="bg-deep-blue/30 border border-forest/20 rounded-xl p-4 mb-6 space-y-2">
            ...
            <select value={createProductId} onChange={(e) => {
              setCreateProductId(e.target.value);
              // Auto-fill name/slug from existing translation if available
              const existing = translations.find((t: any) => t.product_id === e.target.value);
              if (existing) {
                setCreateName(existing.name);
                setCreateSlug(existing.slug);
              } else {
                setCreateName("");
                setCreateSlug("");
              }
            ...
            <input value={createName} onChange={(e) => setCreateName(e.target.value)}
              placeholder="Name (displayed on product page)"
              className="w-full bg-deep-dark border border-silver/10 rounded px-3 py-2 text-sm text-white" />
            <input value={createSlug} onChange={(e) => setCreateSlug(e.target.value.toLowerCase().replace(/\s+/g, "-"))}
              placeholder="Slug (URL path, e.g. xfs145db-wine-cooler)"
              className="w-full bg-deep-dark border border-silver/10 rounded px-3 py-2 text-sm text-white font-mono" />
            <Button size="sm" onClick={createTranslation}>Create Translation</Button>
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-silver/10 text-silver/50 text-xs uppercase tracking-wider">
                ...
                <th className="text-right p-4 w-32">Actions</th>
              </tr>
            </thead>
            <tbody>
              {translations.map((t) => (
                <tr key={t.id} className="border-b border-silver/5 hover:bg-white/5 group">
                  {editing === t.id ? (
                    ...
                    <>
                      ...
                      <td className="p-4">
                        <input value={editName} onChange={(e) => setEditName(e.target.value)}
                          className="w-full bg-deep-dark border border-silver/10 rounded px-2 py-1 text-xs text-white" />
                      </td>
                      <td className="p-4">
                        <input value={editSlug} onChange={(e) => setEditSlug(e.target.value.toLowerCase().replace(/\s+/g, "-"))}
                          className="w-full bg-deep-dark border border-silver/10 rounded px-2 py-1 text-xs text-white font-mono" />
                      </td>
                      <td className="p-4">
                    ...
                    <>
                      ...
                      <td className="p-4 text-silver/50 text-xs truncate max-w-[200px]">{t.description || "—"}</td>
              ...
              {translations.length === 0 && (
                <tr><td colSpan={5} className="p-8 text-center text-xs text-silver/40">No translations for "{locale}". Click "Add Translation" to create one.</td></tr>
              )}
            </tbody>
          </table>
        </div>

        {/* Meta Fields Section (below table when editing) */}
        {editing && (
          <div className="mt-6 bg-deep-blue/30 border border-silver/10 rounded-xl p-6">
            ...
            <div className="space-y-3">
              ...
              <input value={editOgImage} onChange={(e) => setEditOgImage(e.target.value)}
                ...
                className="w-full bg-deep-dark border border-silver/10 rounded px-3 py-2 text-sm text-white font-mono text-xs" />
              <div className="flex gap-2">
