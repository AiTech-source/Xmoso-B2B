...
export async function GET() {
  try {
    ...
    if (!rt) return NextResponse.json({ error: "No GSC refresh token. Visit /api/seo/gsc/auth" });
    ...
    return NextResponse.json({
      error: "GSC not accessible with current account. Re-authorize with the Google account that OWNS xmoso.com in Search Console.",
      action: "Visit /api/seo/gsc/auth and use the correct Google account.",
    }, { status: 403 });
  } catch (e: any) {
