...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (supabase) {
    const [{ data }, banners] = await Promise.all([
      ...
      getInitialPageBanners(supabase, "contact"),
    ]);
    pageData = data;
  ...
  return (
    <>
      ...
      <main style={{ paddingTop: "64px" }}>
        {pageData?.show_banner !== false && <PageBannerCarousel pageKey="contact" vignette={pageData?.vignette_enabled !== false} initialBanners={initialBanners} />}
