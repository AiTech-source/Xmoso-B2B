...
export const generateStaticParams = generateProductSlugParams;
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${translation.meta_title || translation.name} | Wine Cooler OEM China — Xmoso Factory Direct`,
    description: translation.meta_description || translation.description || `${translation.name} — custom wine cooler OEM from Xmoso China factory. CE/UL/ETL certified.`,
    alternates: generateAlternates(`/products/${slug}`, locale),
    openGraph: {
      type: "website",
      title: `${translation.meta_title || translation.name} | Xmoso OEM`,
...
export default async function ProductDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  // Merge locale-specific section overrides from translation content.sections
  const sectionsOverride = translation.content?.sections || {};
  const counters = sectionsOverride.paramCounters || product.param_counters || [];
  const tabs = sectionsOverride.specTabs || product.spec_tabs || [];
  const eco = sectionsOverride.ecoFeatures || product.eco_features || [];
  const highlights = sectionsOverride.highlights || product.highlights || [];
                  ...
                  {related.map((p: any) => (
                    <a key={p.id} href={`/${locale}/products/${p.slug}`}
                      ...
                      <div className="aspect-square bg-deep-dark/40 rounded-lg overflow-hidden mb-3 flex items-center justify-center">
                        {p.image ? (
                          <img src={cdnUrl(p.image)} alt={p.name} className="w-full h-full object-contain p-3" width={200} height={200} loading="lazy" />
                        ) : (
