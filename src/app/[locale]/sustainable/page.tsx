...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title, description: desc, alternates: generateAlternates("/sustainable", locale), openGraph: { type: "website", title, description: desc, images: [{ url: ogUrl }] } };
}
...
export default async function SustainablePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (supabase) {
    const [{ data }, banners] = await Promise.all([
      supabase.from("page_contents").select("*").eq("page_key", "sustainable").eq("locale", locale).maybeSingle(),
      getInitialPageBanners(supabase, "sustainable"),
    ]);
    pageData = data;
  ...
  return (
    <>
      ...
      <main style={{ paddingTop: "64px" }}>
        <PageBannerCarousel pageKey="sustainable" vignette initialBanners={initialBanners} />
