...
export default function PageContentRenderer({ content }: { content: { blocks?: Block[] } }) {
            ...
            return (
              <div key={i} className="my-8" style={{ maxWidth: imgMaxW, margin: "32px auto" }}>
                <div className="relative rounded-xl overflow-hidden img-vignette" style={{
                  borderRadius: "12px",
                  maxHeight: imgMaxH,
                  aspectRatio: block.data.aspectRatio || "16/9",
                }}>
                  <img src={block.data.url} loading="lazy" alt={block.data.alt || ""} width="800" height="450" className="w-full h-full object-cover" style={{ maxHeight: imgMaxH }} />
                  {/* Overlay text */}
              ...
              <div key={i} className="flex flex-col md:flex-row gap-6 md:gap-10 my-8 items-start">
                {block.data.image_url && (
                  <div className="md:w-2/5 w-full flex-shrink-0">
                    <img src={block.data.image_url} alt={block.data.image_alt || ""} width="400" height="300" className="w-full h-auto rounded-xl object-cover" style={{ maxHeight: "360px", aspectRatio: "4/3" }} />
                  </div>
                )}
              ...
              <div key={i} className="grid gap-6 my-10" style={{ gridTemplateColumns: `repeat(${Math.min(cols, 4)}, 1fr)` }}>
                {items.map((item: any, idx: number) => (
                  <div key={idx} className="p-6 bg-deep-blue/30 border border-silver/10 rounded-xl text-center hover:border-forest/30 transition-colors">
                    {item.image_url && <img src={item.image_url} alt="" width="400" height="160" className="w-full h-32 object-cover rounded-lg mb-4" style={{ aspectRatio: "5/2" }} />}
