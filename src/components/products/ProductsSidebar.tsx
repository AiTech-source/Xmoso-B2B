      ...
      <nav className="w-10 shrink-0 max-md:hidden">
        <div className="sticky top-24 flex flex-col items-center pt-1 gap-1">
          <button onClick={() => setCollapsed(false)}
            className="text-silver/40 hover:text-forest transition-colors py-2 text-sm" title="Expand sidebar">☰</button>
          {typeGroups.map((g) => {
            const typeId = typeAnchor(g.name);
            return (
              <button key={g.name} onClick={() => onScrollTo(typeId)} title={g.name}
                className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs font-medium transition-all ${
                  activeType === typeId ? "text-forest bg-forest/12" : "text-silver/30 hover:text-silver/60 hover:bg-white/5"
                }`}>{g.name.charAt(0).toUpperCase()}</button>
            );
          })}
        </div>
      </nav>
      ...
      <div className="sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto space-y-0.5 pr-1
        ...
        <div className="flex items-center justify-between mb-3 pb-2 border-b border-silver/10">
          ...
          <button onClick={() => setCollapsed(true)}
            className="text-silver/30 hover:text-forest transition-colors text-xs px-1" title="Collapse sidebar">◀</button>
        </div>
        {typeGroups.map((g) => {
          const typeId = typeAnchor(g.name);
          ...
          return (
            <div key={g.name} className="mb-1.5">
              <button onClick={() => onScrollTo(typeId)}
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${
                  activeType === typeId ? "bg-forest/12 text-forest" : "bg-silver/8 text-silver/40"
                }`}>{prodCount}</span>
              </button>
              {g.categories.length > 1 && (
                <div className="ml-3 mt-0.5 space-y-[1px] border-l border-silver/10 pl-2.5">
                  {g.categories.map((c) => {
                    ...
                    return (
                      <button key={c.id} onClick={() => onScrollTo(catId)}
                        className={`block text-xs py-1 px-2 rounded-md transition-all duration-200 w-full text-left ${
                          activeCat === catId ? "text-ice font-medium bg-ice/6" : "text-silver/50 hover:text-silver/80"
                        }`}>{c.name}</button>
                    );
                  })}
                </div>
...
// ─── Mobile Drawer (no framer-motion) ───
    ...
    <div className="md:hidden">
      ...
      {!open && (
        <div className="fixed left-0 top-1/2 -translate-y-1/2 z-40 transition-all duration-300"
          style={{ transform: open ? "translateX(0)" : "translateX(0)" }}>
      ...
      {/* Drawer */}
      <div className={`fixed left-0 top-0 bottom-0 z-50 w-[280px] bg-[#0E0E18]/98 backdrop-blur-xl shadow-2xl border-r border-silver/8 flex flex-col transition-transform duration-300 ease-out ${
        ...
        <div className="flex items-center justify-between px-5 py-5 border-b border-silver/8">
          ...
          <button onClick={() => setOpen(false)}
            className="w-7 h-7 flex items-center justify-center rounded-full bg-white/5 text-silver/60 hover:text-white hover:bg-white/10 transition-all text-sm cursor-pointer">✕</button>
        </div>
          ...
          {typeGroups.map((g) => {
            ...
            return (
              <div key={g.name} className="mb-1">
                <button onClick={() => { onScrollTo(typeId); setOpen(false); }}
                  className={`w-full flex items-center justify-between text-sm py-2.5 px-3 rounded-xl transition-all duration-200 text-left ${
                    activeType === typeId ? "text-forest font-medium bg-forest/6" : "text-white/80 active:text-white"
                  }`}>
                  <span>{g.name}</span>
                  <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                    activeType === typeId ? "bg-forest/12 text-forest" : "bg-silver/10 text-silver/50"
                  }`}>{prodCount}</span>
                </button>
                {g.categories.length > 1 && (
                  <div className="ml-4 pl-3 border-l border-silver/10 space-y-[1px]">
                    {g.categories.map((c) => {
                      const catId = typeId + "-c-" + c.id;
                      return (
                        <button key={c.id} onClick={() => { onScrollTo(catId); setOpen(false); }}
                          className={`block text-xs py-2 px-3 rounded-lg transition-all duration-200 w-full text-left ${
                            activeCat === catId ? "text-ice font-medium bg-ice/8" : "text-white/70 hover:text-white"
                          }`}>{c.name}</button>
                      );
                    })}
                  </div>
        ...
        <div className="px-5 py-3 border-t border-silver/8 text-center">
          ...
          <p className="text-[10px] text-silver/40 mt-1.5 tracking-wider">← Drawer</p>
        </div>
      </div>
    </div>
  );
...
export default function ProductsSidebar({ typeGroups }: { typeGroups: TypeGroup[] }) {
  ...
  return (
    <>
      <DesktopSidebar typeGroups={typeGroups} activeType={activeType} activeCat={activeCat} onScrollTo={scrollTo} />
      <MobileDrawer typeGroups={typeGroups} activeType={activeType} activeCat={activeCat} onScrollTo={scrollTo} />
    </>
  );
}
